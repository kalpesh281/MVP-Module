...
import InfoTip from '../Components/Extra/InfoTip';

/**
 * The published methodology.
 *
 * Every number on this page is fetched from `/api/methodology`, which
 * reads the same constants that scored the company reading it. Nothing
 * here is transcribed, so the page and the rubric cannot drift apart —
 * which is the entire claim. A scoring system nobody can check is
 * indistinguishable from one that was invented.
 *
 * It is a public URL on purpose. The rubric is the thing we give away;
 * the scan history is the product.
    ...
    >
      <motion.header variants={fadeUp} className="space-y-3">
        ...
        <p className="max-w-2xl leading-relaxed text-ink-muted">
          ...
          grade — not copied here by hand. Scoring rules{' '}
          <Version value={data.rubric_version} /> and rate card{' '}
          <Version value={data.rate_version} />.
        </p>
      ...
      <motion.section variants={fadeUp} className="space-y-4">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-ink-muted">
          ...
          <strong className="font-medium text-ink">
            {data.suppress_above_inconclusive} inconclusive points
          </strong>{' '}
...
function Version({ value }) {
  return <span className="font-mono text-[0.9em] text-ink">{value}</span>;
}
