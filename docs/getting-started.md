...
|---|---|---|---|---|
| 5 | **Cert Spotter API key** | sslmate.com/certspotter | **Free** — 100 requests/hour | `CERTSPOTTER_API_KEY`. The primary certificate-transparency source for the subdomain check. Without it we fall back to crt.sh, which answered roughly **1 time in 6** in testing and takes 3.8–8.5 s against Cert Spotter's 1.2–2.6 s. Under rubric v1.0 a CT outage blanked the grade entirely; v1.1 fixed that, so a failure now costs the subdomain check's 23 points out of the denominator rather than the whole grade. Still the difference between a usable answer and a partial one on most scans. |
